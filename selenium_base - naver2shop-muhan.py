...
#                print('새벽시간 이므로',addTime,'(초) 만큼 시간이 추가됨')
        ...
        if _timeRule[0][0]+1 <= allCount:
            H = int(DateToString('now').split('-')[-1])
            if 7 <= H and 8 >= H:
                allCount = 0
            else:
                print('시도 횟수', allCount)
                print('하루 검색 횟수 초가 : 다음날 아침 7시에 다시 시작됨')
                time.sleep(600)
                continue
...
#        wordKey = random.randrange(0, len(words))
        ...
        for number in range(len(words)):
            ...
            text = words[number][0]
            sent_arr = text.split(' ')
            a = list(permutations(sent_arr, len(sent_arr)))
            
            print('생성된 키워드 경우의 수 :',len(a))
            wordKey = random.randrange(0, len(a))
    ...
    #        https://m.shopping.naver.com/home/m/index.naver
            ...
            driver.get('https://m.shopping.naver.com/home/m/index.naver')
    ...
    #        itemTime =  random.randrange(30, 60)
            itemTime =  random.randrange(3, _timeRule[1][0]+addTime)
